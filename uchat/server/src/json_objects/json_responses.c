...
void edit_room_desc_response(cJSON *j_request, t_client *client) {
    ...
    if (get_member_type(client->info->database, client->user->user_id, room_id)
        != DB_ORDINARY || is_valid_message(room_desc) == false) {
        return;
    }
...
void get_member_info_response(cJSON *j_request, t_client *client) {
    ...
    cJSON_AddNumberToObject(j_response, "type", RQ_MEMBER_INFO);
...
void edit_desc_response(cJSON *j_request, t_client *client) {
    gchar *new_desc = vm_get_valuestring(j_request, "desc");
    cJSON *j_response = cJSON_CreateObject();
    gchar *j_data = NULL;
    
    if (is_valid_message(new_desc) == false) {
        return;
    }

    edit_user_desc(client->info->database, client->user->user_id, new_desc);
    cJSON_AddNumberToObject(j_response, "token", RQ_UPD_USER_DESC);
...
static bool is_valid_msg_rq(gchar *msg, sqlite3 *db, gint user_id, gint room_id) {
    return is_valid_member(db, user_id, room_id) && is_valid_message(msg);
}
...
void search_msg_response(cJSON *j_request, t_client *client) {
    gchar *search_msg = vm_get_valuestring(j_request, "msg");
    gint room_id = vm_get_object(j_request, "room_id")->valueint;
    cJSON *j_response = cJSON_CreateObject();
    cJSON *j_msgs = NULL;
    gchar *j_data = NULL;
...
void edit_message_response(cJSON *j_request, t_client *client) {
    gchar *msg_str = vm_get_valuestring(j_request, "msg");
    gint room_id = vm_get_object(j_request, "room_id")->valueint;
    ...
    if (is_member(client->info->database, client->user->user_id, room_id) &&
        is_msg_owner(client->info->database, client->user->user_id, room_id) &&
        get_member_type(client->info->database, client->user->user_id, room_id) != DB_BANNED) {
        
        edit_message_by_id(client->info->database, client->user->user_id, msg_str);

        cJSON_AddNumberToObject(j_response, "token", RQ_EDIT_MSG);
        cJSON_AddNumberToObject(j_response, "room_id", room_id);
        cJSON_AddNumberToObject(j_response, "msg_id", msg_id);
        cJSON_AddStringToObject(j_response, "msg", msg_str);
...
void sign_in_response(cJSON *j_request, t_client *client) {
    ...
    if (check == NULL) {
        ...
        client->user = check;
        // //fprintf(stdout, "user_id after init x2: %d\n", client->user->user_id);
        valid_status = "true";
    ...
    cJSON_AddStringToObject(j_response, "auth_token", MX_J_STR(user->auth_token));
    cJSON_AddStringToObject(j_response, "login", MX_J_STR(user->login));
    cJSON_AddStringToObject(j_response, "desc", MX_J_STR(user->desc));
