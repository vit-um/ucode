...
static void find_com_sub(t_com_sub *com_sub, int i);
...
static void insert_com_sub(t_com_sub *c, char **data, t_ush *ush) {
    c->temp_str = util_strdup(*data, c->space_first_index + 1, c->space_end_index);
    c->status = execute_command(ush, c->temp_str, 1, &c->cout_execute);
